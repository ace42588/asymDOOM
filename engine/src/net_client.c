...
static void NET_CL_ParseGameStart(net_packet_t *packet)
{
    ...
    if (!NET_ReadSettings(packet, &settings)) {
        ...
        printf("asym: gamestart read failed\n");
        return;
    }

    // Dedicated auto-launch can deliver GAMESTART immediately after
    // (or, if reordered, instead of) LAUNCH. Treat that as an implicit launch.
    if (client_state == CLIENT_STATE_WAITING_LAUNCH) {
        client_state = CLIENT_STATE_WAITING_START;
    }
    ...
    if (client_state != CLIENT_STATE_WAITING_START) {
        ...
        printf("asym: gamestart ignored state=%d\n", client_state);
        return;
    }

    if (settings.num_players > NET_MAXPLAYERS || settings.consoleplayer >= NET_MAXPLAYERS) {
        // insane values
