...
// Late joiners stay out of playeringame[] until this tic so every peer
// applies the possess event together.
static int net_join_tic = 0;
...
void D_StartNetGame(net_gamesettings_t *settings, netgame_startup_callback_t callback)
{
    ...
    net_join_tic = settings->asym_join_tic;
...
boolean D_InitNetGame(net_connect_data_t *connect_data)
{
    ...
    if (addr != NULL) {
        ...
        printf("asym: waiting launch\n");
        NET_WaitForLaunch();
        printf("asym: got launch\n");
...
void TryRunTics(void)
{
    ...
    if (new_sync) {
        ...
        // Fast-forward a late joiner without stalling the browser for
        // tens of thousands of tics in one frame.
        if (counts > 64) counts = 64;
    }
    else {
        // decide how many tics to run
    ...
    {
        static boolean catching_up = false;
        boolean now_catching = availabletics > 20;
        if (now_catching && !catching_up) {
            printf("asym: catchup 1\n");
            catching_up = true;
        }
    ...
    while (!PlayersInGame() || lowtic < gametic / ticdup + counts) {
        ...
        if (lowtic < gametic / ticdup + counts) {
            ...
            // emscripten_sleep inside the rAF main loop prevents the next
            // frame from being scheduled (ASYNCIFY never returns to
            // set_main_loop). Yield by returning; the next rAF retries.
            return;
        }
    }
