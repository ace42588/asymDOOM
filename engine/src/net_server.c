...
typedef struct {
    ...
    // asymDOOM late join: tic at which this player becomes playeringame.
    // Original starters use 0. Catch-up clients ignore their own ticcmds
    // until acknowledged passes this point.
    unsigned int join_tic;
    boolean late_join;
    boolean late_launch_sent;
...
// Full-match ticcmd history so late joiners can fast-forward from tic 0.
static net_full_ticcmd_t *tic_history = NULL;
static int tic_history_len = 0;
static int tic_history_cap = 0;
static boolean auto_launch_sent = false;
...
{
    unsigned int lowtic = (unsigned int)-1;
    int i;
    boolean any = false;

    for (i = 0; i < MAXNETNODES; ++i) {
        if (!ClientConnected(&clients[i])) continue;
        // Catching-up late joiners must not stall the live recv window.
        if (ClientCatchingUp(&clients[i])) continue;
        any = true;
    ...
    if (!any) return recvwindow_start;
    return lowtic;
}
...
static void NET_SV_AdvanceWindow(void)
{
    ...
    while (recvwindow_start < lowtic) {
        ...
        for (i = 0; i < NET_MAXPLAYERS; ++i) {
            ...
            if (sv_players[i]->join_tic > recvwindow_start) {
                continue;
            }
...
static void NET_SV_InitNewClient(net_client_t *client, net_addr_t *addr, net_protocol_t protocol)
{
    ...
    client->ready = false;
    client->join_tic = 0;
    client->late_join = false;
    client->late_launch_sent = false;
...
static boolean PlayerLiveAt(int player, unsigned int seq)
{
    net_client_t *c = sv_players[player];

    if (c == NULL || !ClientConnected(c) || c->drone) return false;
    return seq >= c->join_tic;
}

static void NET_SV_HistoryStore(unsigned int seq, const net_full_ticcmd_t *cmd)
{
    if ((int)seq < tic_history_len) {
        tic_history[seq] = *cmd;
        return;
    }
    if ((int)seq >= tic_history_cap) {
        int ncap = tic_history_cap ? tic_history_cap * 2 : 512;
        net_full_ticcmd_t *grown;

        while (ncap <= (int)seq) ncap *= 2;
        grown = realloc(tic_history, (size_t)ncap * sizeof(*tic_history));
        if (grown == NULL) {
            I_Error("NET_SV_HistoryStore: out of memory recording tic %u", seq);
        }
        tic_history = grown;
        tic_history_cap = ncap;
    }
    while (tic_history_len < (int)seq) {
        memset(&tic_history[tic_history_len], 0, sizeof(*tic_history));
        tic_history[tic_history_len].seq = (unsigned int)tic_history_len;
        tic_history_len++;
    }
    tic_history[seq] = *cmd;
    tic_history_len = (int)seq + 1;
}

static void NET_SV_HistoryReset(void)
{
    free(tic_history);
    tic_history = NULL;
    tic_history_len = 0;
...
static boolean NET_SV_BuildCanonicalTic(unsigned int seq, net_full_ticcmd_t *cmd)
{
    int index = (int)seq - (int)recvwindow_start;
    int i;

    if (index < 0 || index >= BACKUPTICS) return false;
...
static int NET_SV_AllocPlayerSlot(net_client_t *client)
{
    int pl;

    // Prefer demon slots (1+) so a late joiner never steals the marine
    // mid-match. Slot 0 is reused only if the marine already left.
    for (pl = 1; pl < NET_MAXPLAYERS; ++pl) {
        if (sv_players[pl] == NULL || !ClientConnected(sv_players[pl])) {
            sv_players[pl] = client;
            client->player_number = pl;
            return pl;
        }
    }
    if (sv_players[0] == NULL || !ClientConnected(sv_players[0])) {
        sv_players[0] = client;
...
static void NET_SV_StartLateJoin(net_client_t *client)
{
    ...
    join_tic = recvwindow_start + 8;
    client->join_tic = join_tic;
    client->sendseq = 0;
    client->acknowledged = 0;
    client->ready = true;
    nowtime = I_GetTimeMS();
    ...
    settings.consoleplayer = client->player_number;
    settings.asym_join_tic = (int)join_tic;
    // num_players stays the original start count so the joiner simulates
    // from tic 0 with only the marine in-game until join_tic.

    startpacket = NET_Conn_NewReliable(&client->connection, NET_PACKET_TYPE_GAMESTART);
    NET_WriteSettings(startpacket, &settings);

    NET_Log("server: late join player %d at tic %u (history %d)", client->player_number, join_tic, tic_history_len);
}
...
static void NET_SV_ParseSYN(net_packet_t *packet, net_client_t *client, net_addr_t *addr)
{
    ...
    if (server_state == SERVER_IN_GAME && !data.drone) {
        if (NET_SV_AllocPlayerSlot(client) < 0) {
            NET_SV_SendReject(addr, "Server is full!");
            client->active = false;
            return;
        }
        client->late_join = true;
        NET_Log("server: late join assigned player %d", client->player_number);
    }
...
static void NET_SV_ParseLaunch(net_packet_t *packet, net_client_t *client)
{
    ...
    auto_launch_sent = true;
}

static void NET_SV_AutoLaunch(void)
{
    net_packet_t *launchpacket;
    int num_players;
    unsigned int i;

    if (auto_launch_sent || server_state != SERVER_WAITING_LAUNCH) return;
    if (NET_SV_NumPlayers() < 1) return;

    NET_Log("server: auto-launching on first player");
    NET_SV_AssignPlayers();
    num_players = NET_SV_NumPlayers();
...
static void NET_SV_ParseGameStart(net_packet_t *packet, net_client_t *client)
{
    ...
    // Late joiner: they send GAMESTART as "ready" after receiving LAUNCH.
    if (server_state == SERVER_IN_GAME && client->late_join) {
        NET_SV_StartLateJoin(client);
        return;
    }
...
static void NET_SV_ParseGameData(net_packet_t *packet, net_client_t *client)
{
    ...
    // A late joiner's 8-bit ack/seq refer to the start of the match, not
    // the live recv window. Expand against their own acknowledged point
    // and ignore ticcmds until they have caught up.
    if (client->late_join && client->acknowledged + 64 < recvwindow_start) {
        unsigned int catch_ack = NET_ExpandTicNum(client->acknowledged, ackseq);
        if (catch_ack > client->acknowledged) client->acknowledged = catch_ack;
...
static void NET_SV_PumpSendQueue(net_client_t *client)
{
    ...
    while (client->sendseq < (int)recvwindow_start && client->sendseq < tic_history_len && pumped < 8) {
        ...
        NET_SV_SendTics(client, (unsigned int)client->sendseq, (unsigned int)client->sendseq);
        ++client->sendseq;
        ++pumped;
    }
    if (client->sendseq < (int)recvwindow_start) return;

    // If a client has not sent any acknowledgments for a while,
    // wait until they catch up. Signed compare: catching-up sendseq is
    // small while live acknowledged is large.

    if (client->sendseq - (int)NET_SV_LatestAcknowledged() > 40) {
        return;
    }

    // Work out the index into the receive window

    recv_index = client->sendseq - (int)recvwindow_start;
    ...
    if (num_players == 0 && client->sendseq > (int)recvwindow_start + 10) {
        return;
    }

    // Record a canonical copy (all live players) before stripping self.
    if (client->sendseq >= tic_history_len) {
        net_full_ticcmd_t canon;
        if (NET_SV_BuildCanonicalTic((unsigned int)client->sendseq, &canon)) {
            NET_SV_HistoryStore((unsigned int)client->sendseq, &canon);
        }
    }
    ...
    memset(&cmd, 0, sizeof(cmd));
    cmd.seq = (unsigned int)client->sendseq;
    cmd.latency = 0;
    ...
    NET_SV_SendTics(client, (unsigned int)starttic, (unsigned int)endtic);
...
static void NET_SV_RunClient(net_client_t *client)
{
    ...
    if (server_state == SERVER_IN_GAME) {
        if (client->late_join && !client->late_launch_sent && ClientConnected(client)) {
            net_packet_t *launchpacket = NET_Conn_NewReliable(&client->connection, NET_PACKET_TYPE_LAUNCH);
            NET_WriteInt8(launchpacket, sv_settings.num_players);
            client->late_launch_sent = true;
