...
{
    // Browser clients pass -nogui. Opening the textscreen wait dialog
    // creates an SDL window on #canvas and TXT_Shutdown then destroys it,
    // which Emscripten's SDL backend implements by setting the canvas
    // bitmap to 0x0 — leaving a black box. Skip that path and yield with
    // I_Sleep so ASYNCIFY can deliver WebSocket packets.
    const boolean use_gui = !M_ParmExists("-nogui");
