...
void NET_WriteSettings(net_packet_t *packet, net_gamesettings_t *settings)
{
    ...
    NET_WriteInt8(packet, settings->asym_marine_death);
    NET_WriteInt8(packet, settings->asym_demon_death);
    NET_WriteInt32(packet, settings->asym_possess_mask);
    NET_WriteInt8(packet, settings->asym_demon_view);
    NET_WriteInt32(packet, settings->asym_join_tic);
...
boolean NET_ReadSettings(net_packet_t *packet, net_gamesettings_t *settings)
{
    ...
    success = NET_ReadInt8(packet, (unsigned int *) &settings->ticdup)
           ...
           && NET_ReadInt8(packet, (unsigned int *) &settings->asym_marine_death)
           && NET_ReadInt8(packet, (unsigned int *) &settings->asym_demon_death)
           && NET_ReadInt32(packet, (unsigned int *) &settings->asym_possess_mask)
           && NET_ReadInt8(packet, (unsigned int *) &settings->asym_demon_view)
           && NET_ReadInt32(packet, (unsigned int *) &settings->asym_join_tic)
