...
void D_RunFrame()
{
    ...
    // asymDOOM: spectators have no body; listen from the marine instead.
    S_UpdateSounds(players[consoleplayer].mo != NULL ? players[consoleplayer].mo
                                                     : players[0].mo); // move positional sounds
...
void D_DoomLoop(void)
{
    ...
    // Skip the melt wipe on the first frame. Spreading it across rAF
    // frames delayed TryRunTics long enough to stall the sim.
    wipegamestate = gamestate;

    if (playeringame[0] && players[0].mo == NULL) {
        P_SpawnPlayer(&playerstarts[0]);
    }

    // Start the browser loop last. Do not TryRunTics() here: that ran
...
void D_DoomMain(void)
{
    ...
    // Open SDL video before the net wait / map load. Some setup code
    // touches the framebuffer; waiting until D_DoomLoop left the page black.
    // Do not start the emscripten loop yet — D_RunFrame would try to draw
    // GS_LEVEL before G_InitNew has spawned the world.
    if (!M_ParmExists("-dedicated")) {
        I_SetWindowTitle(gamedescription);
        I_GraphicsCheckCommandLine();
        I_InitGraphics();
        printf("asym: graphics init\n");
    }
    ...
    if (gameaction != ga_loadgame) {
        ...
        printf("asym: loading map\n");
        G_InitNew(startskill, startepisode, startmap);
        printf("asym: map loaded\n");
    }
        else
