...
void P_LoadThings (int lump)
{
    ...
    if (!deathmatch)
    {
        // asymDOOM: only the marine (slot 0) needs a player start. Extra
        // slots possess map monsters and Chocolate Doom's MAXPLAYERS=8
        // check would I_Error on shareware maps that only have 4 starts.
        int maxcheck = asym_mode ? 1 : MAXPLAYERS;
        for (i = 0; i < MAXPLAYERS; i++)
        {
            if (i < maxcheck && playeringame[i] && !playerstartsingame[i])
            {
  ...
  skill_t	skill)
{
    ...
    // asymDOOM: restart the deterministic spawn ordinal for this level.
    asym_spawn_counter = 0;
