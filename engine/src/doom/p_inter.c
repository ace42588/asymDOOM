  ...
  mobj_t*	target )
{
    ...
    if (target->player)
    {
	// asymDOOM: a possessed demon died - detach the player and schedule
	// a hop to the next body; the corpse animates like a normal monster.
	if (asym_mode && target->type != MT_PLAYER)
	{
	    ASYM_PossessedKilled (target);
	}
	else
	{
	...
	// asymDOOM: the marine fell.
	if (asym_mode && target->player == &players[0])
	    ASYM_MarineKilled ();
	}
    }
  ...
  int 		damage )
{
    ...
    // asymDOOM: possessed attackers get their damage modifier applied and
    // earn modifier points for damage dealt.
    damage = ASYM_ScaleDamageFrom (source, damage);
    ASYM_AwardDamage (source, target, damage);
    ...
    if ( (!target->threshold || target->type == MT_VILE)
	 ...
	 && source->type != MT_VILE
	 // asymDOOM: AI demons never retaliate against possessed demons;
	 // only the marine is their enemy.
	 && !(asym_mode && ASYM_IsPossessed (source)))
    {
