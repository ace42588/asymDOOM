...
static void RunTic(ticcmd_t *cmds, boolean *ingame)
{
    ...
    // Check for player quits and (asymDOOM) mid-game joins. Both are driven
    // by the per-tic ingame mask from the server, so every peer - including
    // a late joiner replaying the tic history - applies them on the same tic.
...
static void LoadGameSettings(net_gamesettings_t *settings)
{
    ...
    // asymDOOM: rules arrive with the game settings so every peer agrees.
    ASYM_LoadSettings(settings);
    asym_mode = netgame && !deathmatch;
...
static void SaveGameSettings(net_gamesettings_t *settings)
{
    ...
    // asymDOOM: the controlling client supplies the rule settings.
    ASYM_InitFromArgs();
    ASYM_SaveSettings(settings);
}
