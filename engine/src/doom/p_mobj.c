  ...
  mobjtype_t	type )
{
    ...
    // asymDOOM: deterministic spawn ordinal for possession picks.
    mobj->asym_id = asym_spawn_counter++;
...
void P_SpawnPlayer (mapthing_t* mthing)
{
    ...
    // asymDOOM: only slot 0 is a marine. Demon slots possess map monsters
    // (bound after level setup in ASYM_LevelStart), so never spawn extra
    // marine bodies for them.
    if (asym_mode && ASYM_IsDemonSlot(mthing->type-1))
	return;
