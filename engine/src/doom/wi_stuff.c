...
void WI_drawNetgameStats(void)
{
	...
	// asymDOOM: demon slots are hidden on the stats screen.
	if (!plrs[i].in)
	    continue;
...
static void WI_loadUnloadData(load_callback_t callback)
{
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
	// asymDOOM: the IWAD only ships four player patches; extra demon
	// slots reuse them (they are hidden on stats screens anyway).
