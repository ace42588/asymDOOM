...
void G_BuildTiccmd(ticcmd_t *cmd, int maketic)
{
    ...
    if (asym_mode && ASYM_IsDemonSlot(consoleplayer)) {
        // asymDOOM demon controls: number keys buy modifiers (rides the
        // ticcmd arti field so every peer applies the purchase on the same
        // tic), q/e move floaters down/up.
        static boolean buyheld[ASYM_NUM_MODS];

        for (i = 0; i < ASYM_NUM_MODS; ++i) {
            if (gamekeydown['1' + i]) {
                if (!buyheld[i]) {
                    buyheld[i] = true;
                    cmd->arti = i + 1;
        ...
        if (gamekeydown['e']) cmd->lookfly = 1;
        else if (gamekeydown['q']) cmd->lookfly = 2;
    }
...
void G_DoLoadLevel(void)
{
    ...
    // asymDOOM: bind demon players to fresh bodies on the new level.
    if (asym_mode) {
        printf("asym: level start\n");
        ASYM_LevelStart();
    }
...
void G_Ticker(void)
{
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
        if (playeringame[i]) {
            ...
            // asymDOOM: deterministic modifier purchases (ticcmd arti field)
            if (asym_mode && cmd->arti) {
                ASYM_TryBuy(&players[i], cmd->arti);
            }
...
void G_PlayerFinishLevel(int player)
{
    ...
    // asymDOOM: demon players may be between bodies (spectating).
    if (p->mo)
...
void G_DoCompleted(void)
{
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
        // asymDOOM: intermission stats are the marine's story; demon slots
        // are hidden (the classic screens only lay out four players anyway).
        wminfo.plyr[i].in = asym_mode ? (i == 0 && playeringame[i]) : playeringame[i];
