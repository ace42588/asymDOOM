...
{ 
    ...
    /* asymDOOM: overwrite with host-latched marine intent when active */
    if (g_asym_rules) {
        asym_control_fill_marine_cmd(cmd);
    }
} 
...
{ 
    ...
    if (g_asym_rules) {
        asym_rules_on_level_start(g_asym_rules);
    }
} 
...
{ 
    ...
    { 
      case GS_LEVEL: 
	if (g_asym_rules) {
	    asym_rules_ticker(g_asym_rules);
	    asym_control_think_demons();
	}
	P_Ticker (); 
	ST_Ticker (); 
	AM_Ticker (); 
...
//#endif
    ...
    if (g_asym_rules) {
        /* Headless thin host has no intermission UI; WI_Ticker waits forever
           for a keypress. Jump straight to the next map (wrap episode end). */
        if (!secretexit) {
            if (gamemode != commercial && gamemap == 8)
                wminfo.next = 0;
            else if (gamemode != commercial && gameversion == exe_chex && gamemap == 5)
                wminfo.next = 0;
        }
        G_WorldDone();
        return;
