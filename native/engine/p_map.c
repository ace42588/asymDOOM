...
/* When set, PIT_CheckThing ignores solid actors (geometry-only probe). */
int			asym_spawn_probe;
...
boolean PIT_CheckLine (line_t* ld)
{
	...
	if ( !P_MobjSlides(tmthing) && ld->flags & ML_BLOCKMONSTERS )
...
boolean PIT_CheckThing (mobj_t* thing)
{
    ...
    /* Spawn-escape probes only care about walls/doors, not packed mobs. */
    if (asym_spawn_probe)
	return true;
  ...
  fixed_t	y )
{
	...
	/* Players (MF_DROPOFF) and asym-possessed monsters may walk off
	   ledges; AI monsters still refuse dropoffs taller than 24 units. */
	if ( !(thing->flags&(MF_DROPOFF|MF_FLOAT))
	     && thing->asym_controller < 0
    ...
    /* Possessed (player-controlled) monsters: snap onto stepped-up floors
       immediately, matching P_Move / marine stair climbing across split
       P_XYMovement substeps. Floaters keep their own Z. */
    if (thing->asym_controller >= 0
	&& !(thing->flags & MF_FLOAT)
	&& thing->z < tmfloorz)
    {
	thing->z = tmfloorz;
    }
...
boolean PTR_SlideTraverse (intercept_t* in)
{
    ...
    if ( !P_MobjSlides(slidemo) && (li->flags & ML_BLOCKMONSTERS) )
	goto isblocking;
...
void P_SlideMove (mobj_t* mo)
{
  ...
  retry:
    ...
    P_PathTraverse ( leadx, traily, leadx+mo->momx, traily+mo->momy,
		     PT_ADDLINES, PTR_SlideTraverse );
    /* Large-radius bodies sit closer to walls; corner traces can miss.
       A center trace matches marine slide more often. */
    if (bestslidefrac == FRACUNIT+1)
    {
	P_PathTraverse ( mo->x, mo->y, mo->x+mo->momx, mo->y+mo->momy,
			 PT_ADDLINES, PTR_SlideTraverse );
    }
