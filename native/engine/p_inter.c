  ...
  int 		damage )
{
    ...
    /* asymDOOM combat hooks */
    if (g_asym_rules) {
        damage = asym_rules_scale_damage(source, damage);
        asym_rules_award_damage(source, target, damage);
    }
	...
	if (g_asym_rules) {
	    if (target->type == MT_PLAYER) {
	        int killer = (source && source->asym_controller >= 0) ? source->asym_controller : -1;
	        asym_rules_on_marine_death(killer);
	    } else if (target->asym_controller >= 0) {
	        asym_rules_on_demon_death(target->asym_controller);
	    }
	}
