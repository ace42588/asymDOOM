...
/* Possessed bodies keep attack *frames* for look, but vanilla actions
   A_FaceTarget / A_*Attack snap angle toward actor->target and spawn a
   second shot on top of host-driven fire. Orphan controller slots are
   healed back to AI via asym_rules_controller_active. */
extern int asym_rules_controller_active(mobj_t *mo);
#define ASYM_SKIP_POSSESSED(actor) \
    do { if (asym_rules_controller_active(actor)) return; } while (0)
...
void A_Look (mobj_t* actor)
{
    ...
    /* asymDOOM: player-controlled bodies skip AI */
    if (asym_rules_controller_active(actor)) return;
